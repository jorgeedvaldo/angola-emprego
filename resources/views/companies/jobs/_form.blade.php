...
<div class="mb-3">
    <label class="form-label fw-semibold">{{ __('site.vaga_form.pais') }}</label>
    <select name="country_id" class="form-select @error('country_id') is-invalid @enderror">
        @foreach($countries as $country)
            <option value="{{ $country->id }}" @selected((int) old('country_id', $job->country_id ?? \App\Models\Country::idPorOmissao()) === $country->id)>
                {{ $country->bandeira }} {{ $country->nome }}
            </option>
        @endforeach
    </select>
    <div class="form-text">{{ __('site.vaga_form.pais_dica') }}</div>
