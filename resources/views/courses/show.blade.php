        ...
        <div class="row g-5">
             <div class="col-lg-8">
                ...
                <div class="bg-white p-4 rounded-3 shadow-sm">
                    <h3 class="fw-bold mb-4 text-dark border-bottom pb-2">{{ __('site.cursos.conteudo') }}</h3>
                    <div class="list-group list-group-flush">
                        @forelse($course->lessons as $lesson)
                            ...
                            <a href="{{ route('courses.attend', ['slug' => $course->slug, 'lessonSlug' => $lesson->slug]) }}" class="list-group-item list-group-item-action py-3 d-flex justify-content-between align-items-center">
                                ...
                                @else
                                    <span class="badge bg-light text-dark rounded-pill px-3 border">{{ __('site.cursos.assistir') }}</span>
                                @endif
                            </a>
                        @empty
                            <div class="text-center py-4 text-muted">
                                <i class="bi bi-camera-video-off fs-3 mb-2"></i>
                                <p>{{ __('site.cursos.sem_aulas') }}</p>
                            </div>
                        @endforelse
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                <div class="card shadow-sm border-0 sticky-top" style="top: 100px; border-radius: 12px;">
                    <div class="card-body p-4">
                        <h5 class="fw-bold mb-4 text-dark">{{ __('site.cursos.progresso') }}</h5>
                        @auth
                            @php
                            ...
                            @if($percent == 100)
                                <div class="alert alert-success d-flex align-items-center mb-3" role="alert">
                                  ...
                                  <div>{{ __('site.cursos.concluido') }}</div>
                                </div>
                                <div class="d-grid">
                                    <a href="{{ route('courses.certificate', $course->slug) }}" class="btn btn-warning text-white fw-bold py-2"><i class="bi bi-award-fill me-2"></i> {{ __('site.cursos.baixar_certificado') }}</a>
                                </div>
                            @else
                                <p class="text-muted small mb-0"><i class="bi bi-info-circle me-1"></i> {{ __('site.cursos.conclua_para_certificado') }}</p>
                            @endif
                        @else
                            <div class="text-center py-3">
                                <p class="text-muted mb-4">{{ __('site.cursos.entre_para_progresso') }}</p>
                                <div class="d-grid">
                                    <a href="{{ route('login') }}" class="btn btn-primary fw-bold">{{ __('site.cursos.entrar_agora') }}</a>
                                </div>
                            </div>
                        @endauth
                    </div>
